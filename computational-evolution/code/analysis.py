...
class Animation:
    ...
    def __init__(self, times, agents1): #agents2, agents3, predators
    ...
    def update(self, i):
        ...
        if i > (len(self.sizes1) - 1):
            ...
            self.ax.scatter3D(self.sizes1.iloc[i], self.speeds1.iloc[i], self.angle1.iloc[i], s=self.counts1.iloc[i],
                              color='blue', label='Prey')
            self.pop_text1 = self.ax.text(0.9 * 200, 1.02 * 200, 6, 'Prey:' + str(len(self.speeds1.iloc[i])))
    ...
    def animate(self):
        ...
        plt.show()
        # anim.save('genespace_evolution.gif')

...
class Environment:
    ...
    def step(self):
        ...
        if self.num_predators == 0:
            print("RAN OUT OF AGENTS ON STEP:",self.step_count)
        ...
        for agent in self.agent_list:
            ...
            if random.random() < PREY_REP_RATE:
    ...
    def run(self, num_steps, animate=False, take_data=True):
        ...
        for i in tqdm(range(num_steps)):
            ...
            if self.num_predators == 0:
                print("No predators remaining")
                return 0

            if take_data is True and i > 5000:  # not taking data first 5000 steps
    ...
    def record_pop(self):
        ...
        self.pop_data.append([self.step_count, self.num_agents, self.num_predators,
                              self.num_food, self.prey_births, self.predator_births,
                              self.prey_deaths, self.predator_deaths])
    ...
    def save_data(self, data_file_path):
        ...
        pop_df = pd.DataFrame(data=self.pop_data,
                              columns=['Time Elapsed/s', 'Agent Population', 'Predator Population', 'Food Population',
                                       'Prey Births', 'Predator Births', 'Prey Deaths', 'Predator Deaths'])
        ...
        def end():
            """End simulation early with no data"""
